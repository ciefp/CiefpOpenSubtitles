#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpOpenSubtitles/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpOpenSubtitles

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpOpenSubtitles
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpOpenSubtitles
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpOpenSubtitles/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpOpenSubtitles-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#      CiefpOpenSubtitles INSTALLED SUCCESSFULLY        #"
